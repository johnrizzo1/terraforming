...
terraforming help | Select-String terraforming | select-string -NotMatch help | foreach { $command = ($_.ToString() -split '\s+')[2]; Write-Host "Running terraforming $command --use-bundled-cert --profile=default | Out-File -Encoding utf8 -FilePath $command.tf"; Invoke-Expression -Command "terraforming $command --use-bundled-cert --profile=default | Out-File -Encoding utf8 -FilePath $command.tf" }
```
